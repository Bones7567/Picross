Public Class MariosPicross

    Public board As New List(Of List(Of picrossButton))

    ''' <summary>
    ''' Creates a 2d ist of picross buttons
    ''' </summary>
    Public Sub generateRandomBoard(numOfRows As Integer, numOfCols As Integer)
        ...
        For row As Integer = 0 To numOfRows - 1
            ...
            For col As Integer = 0 To numOfCols - 1
                '50/50 chance of being a good space
        ...
        Try
            For row As Integer = 0 To rows - 1
                For col As Integer = 0 To cols - 1
                    ...
                    'add an event handler to the dynamically added button
    ...
    Private Sub btnClicked(btn As picrossButton, e As EventArgs)
        'allowing to click on a button only once
        If btn.clicked Then
            Return
        Else
            btn.clicked = True
    ...
    Function createTopLabel(nums As List(Of Integer), width As Integer) As Label
        ...
        lbl.Text = Strings.Join(
            nums.Select(Function(x) x.ToString()).ToArray(), 'converts our list of int to string
            ControlChars.NewLine 'this is our seperator
            )
        Return lbl
    End Function

    Private Sub MariosPicross_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        generateRandomBoard(10, 10) 'this is responsible for building our data structure with picrossbuttons
